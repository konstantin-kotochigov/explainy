...
4. Результаты будут сохранены в директории `outputs/` в виде отдельных Jupyter Notebooks (`.ipynb`) для каждой темы
   ...
   **Важно:** Имена файлов `.ipynb` автоматически начинаются с заглавной буквы (например, `Dpr.ipynb`, `Rag.ipynb`).
...
5. (Опционально) Если настроен Google Custom Search API, изображения будут автоматически загружены в директорию `outputs/Img/<Код_темы>/` (или `${OUTPUTS_DIR}/Img/<Код_темы>/` если настроена переменная `OUTPUTS_DIR`)
   ...
   **Важно:** Директория изображений называется `Img` (с заглавной буквы), а поддиректории для тем также начинаются с заглавной буквы (например, `Img/Dpr/`, `Img/Rag/`).
...
│   └── Img/                        # Директория с загруженными изображениями (создается автоматически при запуске)
└── README.md                       # Этот файл
```
...
- `outputs/` - Jupyter notebooks с объяснениями (имена файлов начинаются с заглавной буквы, например `Dpr.ipynb`, `Rag.ipynb`)
- `outputs/Img/<Код_темы>/` - загруженные изображения для каждой темы (директория `Img` и поддиректории начинаются с заглавной буквы, например `Img/Dpr/`, `Img/Rag/`)
...
- Поддиректорию `Img/` внутри для хранения изображений (с заглавной буквы)
...
2. **Работа**: При обработке каждой темы приложение:
   ...
   - **Важно:** Директория называется `Img` (с заглавной буквы), а имена поддиректорий тем также начинаются с заглавной буквы (например, `Img/Dpr/`, `Img/Rag/`)
